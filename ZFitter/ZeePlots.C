...
//For53X//
	...
	chain5->SetBranchAddress("invMass_rawSC", &invMass5, &b_invMass5);
...
//For73X//
	...
	chain7->SetBranchAddress("invMass_rawSC", &invMass7, &b_invMass7);
 ...
 */
	...
	t->Divide(2,2);
	THStack stEEGold ("stEEGold","InvMass of Golden e's in EE");
	THStack stEBGold ("stEBGold","InvMass of Golden e's in EB");
	THStack stEEBad ("stEEBad","InvMass of Bad e's in EE");
	THStack stEBBad ("stEBBad","InvMass of Bad e's in EB");
	...
	t->cd(1);
	stEEGold.Draw("nostackL");
//EEinvMassGold7->DrawNormalized("L");
//EEinvMassGold5->DrawNormalized("SAMEL");

	t->cd(2);
	stEBGold.Draw("nostack L");

	t->cd(3);
	stEEBad.Draw("nostack L");

	t->cd(4);
	stEBBad.Draw("nostack L");

t->SaveAs("selectionPlots/ZeeRaw.pdf");
