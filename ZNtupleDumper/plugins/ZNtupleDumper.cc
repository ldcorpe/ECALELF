...
void ZNtupleDumper::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if(eventType==PARTGUN){
	...
	for(pat::ElectronCollection::const_iterator eleIter2 = eleIter1+1;
	  ...
	  TreeSetDiElectronVar(*eleIter1, *eleIter2);
    GetESPlaneRawEnergy(*eleIter1,0);
    GetESPlaneRawEnergy(*eleIter2,0);
...
void ZNtupleDumper::TreeSetSingleElectronVar(const pat::Electron& electron1, int index){
  ...
  esEnergySCEle[index] = electron1.superCluster()->preshowerEnergy();
	esP1EnergySCEle[index] = GetESPlaneRawEnergy(electron1,1);
	esP2EnergySCEle[index] = GetESPlaneRawEnergy(electron1,2);
...
void ZNtupleDumper::TreeSetSingleElectronVar(const reco::SuperCluster& electron1, int index){
  ...
  esEnergySCEle[index] = electron1.preshowerEnergy();
	esP1EnergySCEle[index] = -1;
	esP2EnergySCEle[index] = -1;
...
float ZNtupleDumper::GetESPlaneRawEnergy(const pat::Electron& electron, unsigned int planeIndex){
   ...
   // auto hits_ele = electron.recHits();
		...
		for (unsigned int i =0; i < pfsc.size();i++){
		auto sc_rh = (electron.pflowPreshowerClusters()[i]).hitsAndFractions();
		std::cout << "LC TEST hist Size" <<  hits_ele->size() << std::endl;
		std::cout << "LC TEST sc_hist Size" <<  sc_rh.size() << std::endl;

		for ( auto rh : sc_rh){

		std::cout << "det " << rh.first.det() << " subdet " << rh.first.subdetId()  << std::endl;

		}
		}*/
/*
   for ( EcalRecHitCollection::const_iterator hit =  hits_ele->begin() ; hit!=hits_ele->end() ; hit++ ){
      double hitenergy = 0;
			hitenergy = hit->energy();
			std::cout << "LC TEST rechit energy : " << hitenergy << ", detetcor " << hit->detid().det() << ", subdet " <<  hit->detid().subdetId() <<  std::endl;
    //  sumLC_E += laserHandle_->getLaserCorrection(detitr->first, runTime_) * hitenergy;
      sumE    += hitenergy;
    }*/
		//  avgLCSCEle[index] = sumLC_E / sumE;
