...
private: 
  ...
  Float_t R9Ele5x5[NELE];      // full5x5_showerShape_.r9 
	#ifdef CMSSW_7_0_X
	#endif
...
void ZNtupleDumper::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  int nWP90   = 0; //only WP90
	...
	} else if(eventType==ZEE || eventType==WENU || eventType==UNKNOWN || eventType==SINGLEELE){
		for( pat::ElectronCollection::const_iterator eleIter1 = electronsHandle->begin();
				eleIter1 != electronsHandle->end();
				eleIter1++){
	...
	if (eventType==ZSC || eventType==UNKNOWN){
		...
		if(HighEtaSC1!= EESuperClustersHandle->end()){
			doFill=true;
			std::cout << "HIGH ETA?" <<std::endl;
		//	TreeSetDiElectronVar(*PatEle1, *HighEtaSC1); FIXME
			if(doExtraCalibTree){
...
//	tree->Branch("R9Ele5x5", R9Ele5x5, "R9Ele5x5[2]/F");
//	#ifdef CMSSW_7_0_X
 // #endif
...
void ZNtupleDumper::TreeSetSingleElectronVar(const pat::Electron& electron1, int index){
	...
	//R9Ele5x5[index] =electron1.full5x5_r9() ;
	//#ifdef CMSSW_7_0_X
	//#endif
	//   if(isMC){
	//     if(electron1.isEB()) 
